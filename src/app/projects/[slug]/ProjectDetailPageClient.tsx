...
function InfoList({ title, items }: { title: string; items: string[] }) {
  return (
    <motion.div variants={fadeUp} className="border border-[var(--site-border)] bg-[var(--site-card)] p-5 backdrop-blur sm:p-6">
...
export default function ProjectDetailPageClient({ slug }: { slug: string }) {
  ...
  return (
    <main className="site-section min-h-screen overflow-hidden pt-28 sm:pt-32" dir={dir}>
      ...
      <div className="mx-auto w-[92%] max-w-7xl pb-16 sm:pb-24">
        ...
        <section className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {facts.map(({ label, value, icon: Icon }) => (
            <motion.div
              ...
              className="border border-[var(--site-border)] bg-[var(--site-card)] p-4 backdrop-blur"
            >
              <Icon className="mb-3 h-5 w-5 text-brand-secondary" />
              <p className="text-xs font-black uppercase tracking-[0.18em] text-brand-gray">{label}</p>
              <p className="mt-2 font-black text-brand-dark">{value}</p>
            </motion.div>
          ))}
        </section>
        ...
        <section className="mt-12 grid gap-5 lg:grid-cols-[0.8fr_1.2fr] lg:items-stretch">
          <div className="border border-[var(--site-border)] bg-[var(--site-card)] p-6 backdrop-blur">
            <p className="text-xs font-black uppercase tracking-[0.25em] text-brand-secondary">Location</p>
            <h2 className="mt-3 text-3xl font-black text-brand-dark">{labels.map}</h2>
            <p className="mt-4 leading-8 text-brand-gray">{details.siteCondition}</p>
            <a
              href={details.mapOpenUrl}
              target="_blank"
              rel="noreferrer"
              className="mt-6 inline-flex items-center gap-2 border border-[var(--site-border)] bg-[var(--site-surface)] px-4 py-3 text-sm font-black text-brand-dark transition hover:border-brand-primary hover:bg-brand-primary hover:text-[#232323]"
            >
              {labels.openMap}
              <ExternalLink className="h-4 w-4" />
            </a>
          </div>
          <div className="min-h-80 overflow-hidden border border-[var(--site-border)] bg-[var(--site-card)] p-2 backdrop-blur sm:min-h-96">
            <iframe
